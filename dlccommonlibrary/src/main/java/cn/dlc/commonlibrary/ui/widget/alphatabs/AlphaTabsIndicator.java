...
public class AlphaTabsIndicator extends LinearLayout {
    ...
    public AlphaTabsIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AlphaTabsIndicator);
            notTabCount = ta.getInt(R.styleable.AlphaTabsIndicator_no_tab_count, 0);
            //LogPlus.e(String.valueOf(notTabCount));
            ta.recycle();
        }
    }
    ...
    private void init() {
        ...
        mChildCounts = getChildCount() - notTabCount;
        ...
        for (int i = 0; i < realChildCount; i++) {
            if (getChildAt(i) instanceof AlphaTabView) {
                ...
                //设置点击监听
                tabView.setOnClickListener(new MyOnClickListener(mTabViews.size() - 1));
            } else {
